...
export const dashboardRouter = createTRPCRouter({
  ...
  getVisitedCountriesWithGeoJson: protectedProcedure.query(async () => {
    ...
    const visitedSet = new Set(countries.map((c) => c.countryCode as string).filter(Boolean));
  ...
  getVisitedCountriesGeoJson: protectedProcedure.query(async () => {
    ...
    const visitedSet = new Set(visited.map((c) => c.countryCode as string).filter(Boolean));
  ...
  getDashboardStats: protectedProcedure.query(async () => {
    ...
    const thisYearPhotosResult = await db
      ...
      .from(photos)
      .where(
        and(gte(photos.dateTimeOriginal, yearStart), sql`${photos.dateTimeOriginal} IS NOT NULL`)
      );
    ...
    // Get total cities
    const citiesResult = await db.selectDistinct({ city: citySets.city }).from(citySets);
    ...
    const lastYearCitiesResult = await db
      ...
      .from(citySets)
      .where(
        and(gte(citySets.createdAt, lastYearStart), sql`${citySets.createdAt} < ${lastYearEnd}`)
      );
