...
export const MapView = () => {
  const trpc = useTRPC();
  const { data } = useSuspenseQuery(trpc.dashboard.getVisitedCountriesWithGeoJson.queryOptions());
      ...
      <div className="lg:col-span-2 rounded-xl overflow-hidden relative h-[400px] lg:h-full">
        {false && (
          <div className="absolute inset-0 bg-background/50 backdrop-blur-sm flex items-center justify-center z-10">
            <div className="text-sm text-muted-foreground">Loading world map data...</div>
          </div>
        )}
        <Mapbox
          id="dashboardMap"
            ...
            <CardContent className="flex-1 flex flex-col p-0 min-h-0">
              <ChartContainer config={chartConfig} className="flex-1 min-h-0">
                ...
                >
                  ...
                  />
                  <Bar dataKey="photoCount" fill="var(--color-photoCount)" radius={5} />
                </BarChart>
              </ChartContainer>
            </CardContent>
            <CardFooter className="shrink-0 flex-col items-start gap-1 text-sm p-0 pt-3">
              <div className="flex gap-2 leading-none font-medium">
                {countriesData.length} countries visited <TrendingUp className="h-4 w-4" />
              </div>
              <div className="text-muted-foreground leading-none">
                Total: {countriesData.reduce((sum, c) => sum + c.photoCount, 0).toLocaleString()}{' '}
                photos
              </div>
            </CardFooter>
          </Card>
