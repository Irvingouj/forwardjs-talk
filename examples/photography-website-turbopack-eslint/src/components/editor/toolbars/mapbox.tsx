...
export const MapboxToolbar = React.forwardRef<HTMLButtonElement>((_, ref) => {
  ...
  const addMap = () => {
    ...
    const validMarkers = markers.filter((m) => !isNaN(m.longitude) && !isNaN(m.latitude));
        ...
        <DialogContent className="sm:max-w-[800px] max-h-[90vh] overflow-y-auto">
          ...
          <div className="h-[450px] w-full rounded-md overflow-hidden border">
            <Mapbox
              ...
              onMarkerDragEnd={(markerId, lngLat) => {
                ...
                setMarkers(
                  markers.map((m) =>
                    m.id === markerId ? { ...m, longitude: lngLat.lng, latitude: lngLat.lat } : m
                  )
                );
              }}
              onMove={(viewState) => {
          ...
          <div className="grid gap-4">
            <div className="flex items-center justify-between">
              <Label className="text-sm font-medium">Markers: {markers.length}</Label>
              <div className="flex gap-2">
                ...
                <Button type="button" size="sm" variant="outline" onClick={addMarker}>
            ...
            <div className="space-y-3 pt-2 border-t">
              ...
              <div className="flex items-center space-x-2">
                <Checkbox
                  ...
                  onCheckedChange={(checked) => setEnableScroll(checked as boolean)}
                />
                <Label htmlFor="enableScroll" className="text-sm font-normal cursor-pointer">
                  Enable scroll zoom
                </Label>
              </div>
              <div className="flex items-center space-x-2">
                ...
                />
                <Label htmlFor="enableZoom" className="text-sm font-normal cursor-pointer">
              ...
              <div className="flex items-center space-x-2">
                <Checkbox
                  id="enableDrag"
                  checked={enableDrag}
                  onCheckedChange={(checked) => setEnableDrag(checked as boolean)}
                />
                <Label htmlFor="enableDrag" className="text-sm font-normal cursor-pointer">
                  Enable drag/pan map
                </Label>
              </div>
            </div>
