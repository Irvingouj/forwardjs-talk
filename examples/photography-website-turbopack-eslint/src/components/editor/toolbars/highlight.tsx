...
const HighlightButton = ({ name, color, isActive, onClick }: HighlightButtonProps) => (
  <button
    onClick={onClick}
  ...
  >
    <div className="flex items-center space-x-2">
      <div className="rounded-sm border px-1 py-px font-medium" style={{ backgroundColor: color }}>
        A
      </div>
      <span>{name}</span>
    </div>
...
export const HighlightToolbar = () => {
  ...
  const handleSetHighlight = (color: string) => {
    editor
      .chain()
      .focus()
      .setHighlight(color === editorState.currentHighlight ? { color: '' } : { color })
      .run();
  };
      ...
      <div className="relative h-full">
        ...
        >
          <ScrollArea className="max-h-80 overflow-y-auto pr-2">
            <div className="mb-2.5 mt-2 px-2 text-xs text-muted-foreground">Background</div>
