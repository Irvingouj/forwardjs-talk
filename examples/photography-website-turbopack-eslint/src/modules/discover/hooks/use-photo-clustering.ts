...
export function usePhotoClustering({ photos, initialZoom = 3 }: UsePhotoClusteringProps) {
  ...
  const handleMove = useCallback(
    (viewState: { zoom: number; latitude: number; longitude: number }) => {
      ...
      const newBounds = calculateBounds(viewState.latitude, viewState.longitude, viewState.zoom);
      setBounds(newBounds);
    },
    []
  );
