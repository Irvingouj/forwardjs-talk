...
export function SectionCardsView() {
  const trpc = useTRPC();
  const { data: stats } = useSuspenseQuery(trpc.dashboard.getDashboardStats.queryOptions());
  return (
    <div className="*:data-[slot=card]:from-primary/5 *:data-[slot=card]:to-card dark:*:data-[slot=card]:bg-card grid grid-cols-1 gap-4 *:data-[slot=card]:bg-linear-to-t *:data-[slot=card]:shadow-xs @xl/main:grid-cols-2 @5xl/main:grid-cols-4">
      <Card className="@container/card">
        ...
        <CardFooter className="flex-col items-start gap-1.5 text-sm">
          <div className="line-clamp-1 flex gap-2 font-medium">
            {stats.thisYearPercentChange >= 0 ? 'Growth this year' : 'Decline this year'}{' '}
            <IconTrendingUp className="size-4" />
          </div>
          <div className="text-muted-foreground">
            {Math.abs(stats.thisYearPercentChange)}%{' '}
            {stats.thisYearPercentChange >= 0 ? 'increase' : 'decrease'} vs last year
          </div>
        </CardFooter>
      </Card>
      <Card className="@container/card">
        ...
        <CardFooter className="flex-col items-start gap-1.5 text-sm">
          <div className="line-clamp-1 flex gap-2 font-medium">
            {stats.thisYearPercentChange >= 0 ? 'Year-over-year growth' : 'Year-over-year decline'}{' '}
            <IconTrendingUp className="size-4" />
          </div>
          <div className="text-muted-foreground">
            {Math.abs(stats.thisYearPercentChange)}%{' '}
      ...
      <Card className="@container/card">
        ...
        <CardFooter className="flex-col items-start gap-1.5 text-sm">
          <div className="line-clamp-1 flex gap-2 font-medium">
            {stats.countriesPercentChange >= 0 ? 'New countries' : 'Countries decreased'}{' '}
            <IconTrendingUp className="size-4" />
          </div>
          <div className="text-muted-foreground">
            {Math.abs(stats.countriesPercentChange)}%{' '}
            {stats.countriesPercentChange >= 0 ? 'increase' : 'decrease'} vs last year
          </div>
        </CardFooter>
      </Card>
      <Card className="@container/card">
        ...
        <CardFooter className="flex-col items-start gap-1.5 text-sm">
          ...
          <div className="text-muted-foreground">
            ...
            {stats.citiesPercentChange >= 0 ? 'increase' : 'decrease'} vs last year
          </div>
        </CardFooter>
      </Card>
    </div>
