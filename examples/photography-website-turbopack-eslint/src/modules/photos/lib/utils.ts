 ...
 */
export const formatExposureCompensation = (exposureCompensation?: number): string => {
 ...
 */
export const formatFocalLength35mm = (focalLength35mm?: number | null): string => {
...
export const getPhotoExif = async (file: File): Promise<TExifData | null> => {
  try {
    ...
    const exifData: TExifData = {
      ...
      focalLength35mm:
        typeof FocalLengthIn35mmFormat === 'number' ? FocalLengthIn35mmFormat : undefined,
      ...
      dateTimeOriginal: DateTimeOriginal ? new Date(DateTimeOriginal * 1000) : undefined,
    };
...
export const getImageInfo = async (file: File): Promise<TImageInfo> => {
  ...
  try {
    ...
    const blurhash = encode(imageData.data, imageData.width, imageData.height, 5, 4);
