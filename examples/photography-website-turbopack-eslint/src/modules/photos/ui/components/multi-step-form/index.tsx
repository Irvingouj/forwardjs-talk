...
export default function MultiStepForm({ className, onSubmit }: MultiStepFormProps) {
  ...
  // Form data
  const [formData, setFormData] = useState<Partial<PhotoFormData>>(INITIAL_FORM_VALUES);
  ...
  const renderStep = () => {
    ...
    switch (step) {
      ...
      case 2:
        return (
          <ThirdStep {...commonProps} onNext={handleNext} onAddressUpdate={handleAddressUpdate} />
        );
      case 3:
          ...
          <AnimatePresence mode="wait">
            ...
            >
              <div className="mb-6">
                ...
                <p className="text-muted-foreground text-sm">{STEP_CONFIG[step].description}</p>
              </div>
