...
export function CityCard({ citySet }: CityCardProps) {
  const { country, city, photoCount, coverPhotoUrl, coverPhotoBlurData } = citySet;
    ...
    <Link href={href} className="block">
      <div className="group relative aspect-3/4 overflow-hidden rounded-3xl bg-muted shadow-lg transition-all hover:shadow-2xl">
        ...
        <div className="absolute top-6 left-6 text-white">
          ...
          <p className="text-lg font-medium drop-shadow-lg opacity-90">{city}</p>
        </div>

        {/* Bottom - Photo Count */}
        <div className="absolute bottom-6 left-6 text-white">
          <p className="text-lg font-medium drop-shadow-lg opacity-90">{photoCount}</p>
        </div>
      </div>
    </Link>
  );
